...
import { generateBookingNumber, generatePaymentNumber, successResponse, errorResponse, paginationMeta } from '../utils/helpers';

const bookings = new Hono<{ Bindings: { DB: D1Database } }>();

// GET /api/bookings - Student's own bookings
bookings.get('/', authMiddleware, async (c) => {
  try {
    ...
    const results = await db.prepare(`
      SELECT b.*, a.name as abhyasika_name, a.address,
...
// ── IMPORTANT: /owner/all MUST come before /:id ──────────────

// GET /api/bookings/owner/all - Owner sees bookings for THEIR abhyasikas only
bookings.get('/owner/all', authMiddleware, requireOwner(), async (c) => {
  try {
    ...
    // owner_id = user.id ensures only OWN abhyasikas
...
bookings.get('/:id', authMiddleware, async (c) => {
  try {
    ...
    // Extra check: owner can only see bookings for their own abhyasikas
    if (user.role === 'owner') {
      const ownerCheck = await db.prepare('SELECT id FROM abhyasikas WHERE id = ? AND owner_id = ?')
        .bind((booking as any).abhyasika_id, user.id).first();
      if (!ownerCheck) return c.json(errorResponse('Booking not found'), 404);
    }
...
bookings.post('/:id/confirm', authMiddleware, async (c) => {
  try {
    const id = c.req.param('id');
    const { gateway_payment_id, gateway_signature } = await c.req.json();
    const db = c.env.DB;
...
// POST /api/bookings/:id/checkin - Owner checks in a student (own abhyasika only)
bookings.post('/:id/checkin', authMiddleware, requireOwner(), async (c) => {
  try {
    ...
    const booking = await db.prepare(`
      SELECT b.id, a.owner_id FROM bookings b
      JOIN abhyasikas a ON a.id = b.abhyasika_id
      WHERE b.id = ?
    `).bind(id).first() as any;

    if (!booking) return c.json(errorResponse('Booking not found'), 404);
    if (user.role !== 'super_admin' && booking.owner_id !== user.id) {
      return c.json(errorResponse('You can only check in students at your own abhyasikas'), 403);
    }
...
// POST /api/bookings/:id/checkout - Owner checks out (own abhyasika only)
bookings.post('/:id/checkout', authMiddleware, requireOwner(), async (c) => {
  try {
    ...
    if (!booking) return c.json(errorResponse('Booking not found'), 404);
    if (user.role !== 'super_admin' && booking.owner_id !== user.id) {
      return c.json(errorResponse('You can only check out students at your own abhyasikas'), 403);
    }
